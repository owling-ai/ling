...
```bash
# Gemini Live；默认“小云朵”，也可选 starlight / moonlamp / honeydrop
GEMINI_API_KEY=...
...
可用 provider 会显示在旧调试台中。`LING_REALTIME_PROVIDER` 可指定默认值：`gemini`、`stepfun`、`minicpm` 或 `volcengine`。Gemini 提供“小云朵 / 小星星 / 月亮灯 / 蜂蜜糖”四个可试听音色，配置与生成方式见 [定制音色方案](./docs/custom-voice.md)。
...
frontend/
  ...
  assets/voices/      Gemini 音色试听 WAV 与生成 manifest
  child/              儿童 PWA
  parent/             家长 PWA
scripts/
  generate_voice_previews.py  通过真实 Gemini Live 重新生成试听
```
...
- `2026-07-11`：加入四个可试听 Gemini 角色音色，默认使用“小云朵”，会话按白名单 profile 建连。
