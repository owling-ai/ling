...
| 模块 | 已实现 | 当前边界 |
|---|---|---|
| 实时交互 | Gemini 童声 RTC、ESP32 PCM 童声网关、StepFun、MiniCPM-o、火山 Ark RTC | 现有硬件链路可联调；正式设备鉴权和二进制协议未实现 |
...
| 硬件 | 现有 ESP32 可通过原 JSON + Base64 WebSocket 收发“小晴天”童声 | 设备身份、绑定和正式二进制协议未实现 |
...
# 现有 ESP32 PCM 固件：独立 ASR -> Gemini 文本 -> 默认“小晴天”24 kHz PCM
...
旧调试台只显示一个“Gemini”入口，并在其中提供“小晴天 / 小青芽”两档可试听童声；浏览器媒体走 ByteRTC。现有 ESP32 继续使用原 `/api/realtime/ws?provider=gemini`，后端以独立流式 ASR 接收 16 kHz PCM、调用 Gemini 文本模型，再把默认“小晴天”合成为 24 kHz PCM 发回。硬件不提交 profile，也不会收到 Gemini Live 原声音频。两档均来自火山官方公版 `seed-tts-2.0` 白名单，不使用真人儿童录音、声音复制或 DSP 升调。完整验证和边界见 [定制音色方案](./docs/custom-voice.md)。
...
浏览器玩偶 -> 会话服务 -> StepFun / MiniCPM-o
                        -> ByteRTC ASR + 打断 + 视频
                              -> Gemini SSE 文本模型
                              -> seed-tts-2.0 原生童声
现有 ESP32 -> JSON/Base64 PCM WebSocket
                        -> 火山流式 ASR -> Gemini 文本模型
                        -> seed-tts-2.0“小晴天”24 kHz PCM
      | 双向转写
      v
...
backend/
  ...
  realtime.py        实时 provider 路由、StepFun / MiniCPM 与 ESP32 童声网关
  speech_asr.py      ESP32 16 kHz PCM 的火山流式 ASR 协议
  gemini_text.py     ESP32 网关的 Gemini 文本生成
  child_tts.py       ESP32 网关的 seed-tts-2.0 24 kHz PCM 合成
...
- `2026-07-11`：为现有 ESP32 PCM WebSocket 加入独立 ASR、Gemini 文本与“小晴天”24 kHz PCM 网关；不再生成或下发 Gemini Live 成人音频。
