...
热路径只有一个 system prompt（拼记忆包，零 LLM 预处理）；
...
2. 复习必须以"分享我的生活"的形态发生：目标词藏在你的生活事件里说出来，
   说英文词后自然带一句中文意思，如「我看到了 panda，就是熊猫呀」。
3. 机会主义触发：孩子主动聊到相关话题，顺势带出对应词。
4. 撤退规则：孩子明显没兴趣（敷衍、转移话题、说"别说英语"），立刻放下复习，
   纯粹陪伴聊天，今天不再尝试。这是"玩偶"和"点读机"的分界线。
...
MASTERY_PROMPT = """判定这场对话里每个英语目标项到达的层级。
层级定义：exposed=玩偶说了、孩子听到了；recognized=孩子表现出听懂（用中文接住语义/回答正确）；
produced=孩子自己说出了这个英文词；none=没出现或孩子明显没接住。
...
输出 JSON（只输出 JSON）：
[{{"item_id": "...", "result": "exposed|recognized|produced|none"}}]"""
