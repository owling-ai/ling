...
# 可用的关系记忆（禁止在刚接通时使用）
{memory_hook}
- 刚接通的第一句话只能是简单问候：可以叫孩子名字、说「我在呢」，但禁止提昨天、记忆、学习或待办，也禁止问任何问题。
- 至少等孩子正常说过两轮后，才可以在自然相关或冷场时轻轻提一次这条记忆；孩子有自己的话题时继续跟随，今天完全不提也没关系。
- 关系记忆不是任务，不能用它要求孩子回答，更不能从关系记忆强行切到学习议程。
...
HOOK_PROMPT = """根据昨天的日记为玩偶生成一条关系记忆线索。它不是开场第一句话，只能在孩子正常聊过两轮后，于自然相关或冷场时轻轻提起。
范例："昨天你说要给那只三角龙起名字，后来起好了吗？"
