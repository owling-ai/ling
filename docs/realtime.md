...
配置 Gemini 童声回调后，旧调试台只显示一个“Gemini”入口，内部使用 `volcengine` 传输；Gemini Live 原声保留为服务端回退能力，不再与童声并列成第二个 Gemini Tab。未配置回调时才显示原有 Gemini Live 或火山 Ark RTC。
...
网页调试台把试听选择保存在 `localStorage`，并可在 `/api/volcengine/prepare` 时发送公开 profile ID。RTC 或硬件客户端不需要传音色字段：服务端会直接采用 `LING_VOLC_VOICE_PROFILE`，默认 `sunny`。服务端把 profile 固定为 `seed-tts-2.0` voice 和持久语气上下文，通话中不能切换；非法 ID 同样回退到 `sunny`。
